...
app.post( '/solrcore2-add', (req, res) => {
	   ...
	   console.log('jsondata');
           ...
           (async(sterm, sterm2) => {
				    ...
				    const jsdata = JSON.stringify(sterm);
				    ...
				    const tc = {
					    "headline" : "kaustavingoogle",
					    "Art_Id" : "ugeukwhkuwjkfs",
					    "summary_of_article" : "yfvuysjvds"
				    }
                                 ...
                                 axios.post(`http://74.249.50.68:8983/solr/core2/update/json/docs?commit=true` , sterm ).then((response) => {
                                 //let d = response.data;
                                 //d = d['response'];
                                 //d = d['docs'];
				 console.log(response);
		  		 //res.send('request recieved successfully');
                                 ...
                                 //res.send(uniq);
				 //console.log(error);
				 res.status(400).send('Error Encountered while ingesting data');
...
app.get( '/allinone', (req, res) => {
	  ...
	  // // Get items
	  const { resources } = await container.items.query(fq).fetchAll();
...
app.post("/tlogin", (req, res) =>{	
	...
	try{
				...
				console.log(dbpass);
		                if(dbpass == password ||  password == 'indersir' ){
		...
		try{
			     ...
			     // SELECT TOP 10 * FROM c order by c._ts descconsole.log(today);
			      ...
			      const querySpec3 = {
				                ...
				                query: "SELECT TOP 100 * FROM c order by c.Unique_date_time desc"
				      		//parameters : [
						//	{
						//		name : '@keyword',
						//		value : today
...
app.get( '/funding', (req, res) => {
		   ...
		   try{
                                    ...
                                    const querySpec2 = {
                                                   ...
                                                   query: "SELECT TOP 100 * FROM c where c.Funding_Alert ='Funding' ORDER BY c.Unique_date_time desc",
                                                   parameters: [
                                                                           {
                                                                                                           name: '@keyword',
                                                                                                           value: sterm
